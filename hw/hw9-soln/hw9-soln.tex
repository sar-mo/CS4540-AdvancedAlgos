...
%Custom Title Fields
\newcommand{\hmwkTitle}{Homework 9 Solutions}
...
\subsection*{Solution}
  \begin{enumerate}
    \item The state space is all possible ways to choose $k$ balls to place on
      $n$ vertices. There are $\binom{n}{k}$ of these.
    \item We show that any state $x$ can reach another state $y$ by
      ...
      \textsc{Base case:} if $x$ and $y$ differ in $0$ balls, we are done.

        \vspace{2mm}
      \textsc{Inductive step:} Say $x$ and $y$ differ in $m$ locations. Let vertex s be
    ...
    \item If we allow multiple balls per vertex, we may interpret each ball as
      ...
      $v_2$,\ldots is
      \[ {\binom{k}{j_1, j_2, \ldots j_n}} \prod_i
    ...
    \item Here, we eliminate a large portion of the state space above - we
      ...
      $\frac{1}{Z}\prod_j deg(v_{i_j})$, where $Z$ is $\sum_{all\ {\binom{n}{k}}\
